...
def auth_gsheet(SERVICE_ACCOUNT_FILE,
...
def connect_to_sheet(sheet_name, SPREADSHEET_ID, service):
...
def load_gsheet(SPREADSHEET_ID, SERVICE_ACCOUNT_FILE):
    service = auth_gsheet(SERVICE_ACCOUNT_FILE)
    sheet_names = get_sheet_names(SPREADSHEET_ID, service)
    data_dict = {}
    for sheet in sheet_names:
        df = connect_to_sheet(sheet, SPREADSHEET_ID, service)
        data_dict[sheet] = df
    return data_dict


def pre_process_surf_data(surf_data_dict):
